...
String& String::insert(size_t pos, const String& str) {
    ...
    // shift all char after pos to the right by the size of str
...
size_t String::find_first_of(const String& str, size_t pos) {
    ...
    for (size_t i = 0; i < str.size(); ++i) {
        size_t ith = find(str.substr(i,1), pos);
        if (ith < first)
            first = ith;  
    }
    return first;
...
size_t String::find_last_of(const String& str, size_t pos) {
    ...
    // the range that should be operated on
    String range = substr(0, pos+1);

    // reverse the range to find the last occurance
    String reversed = range;
    size_t i = 0;
    for (size_t j = range.size()-1; j != npos; --j) {
        reversed[i] = range[j];
        ++i;
    }

    size_t index = reversed.find_first_of(str);
    if (index == npos)
        return npos;
    else
        return pos - index;
